    ...
    public String searchProfessors(@RequestParam(required = false) String professor, Model model) {
        if (professor != null && !professor.isEmpty()) {
            try {
                ...
                // If only one author found → redirect to graph page directly
                if (professors.length == 1) {
                    String authorId = professors[0].getId();
                    return "redirect:/professor-graph?id=" + URLEncoder.encode(authorId, StandardCharsets.UTF_8);
                }
